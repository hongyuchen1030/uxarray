...
from .helpers import _angle_of_2_vectors, _within, _normalize_in_place, _convert_node_xyz_to_lonlat_rad, _convert_node_lonlat_rad_to_xyz, _pt_within_gcr
...
def max_latitude_rad(v1, v2):
    ...
    v1_cart = _convert_node_lonlat_rad_to_xyz(np.deg2rad(v1))
    v2_cart = _convert_node_lonlat_rad_to_xyz(np.deg2rad(v2))
    v_temp = np.cross(v1_cart, v2_cart)
    v0 = np.cross(v_temp, v1_cart)
    v0 = _normalize_in_place(v0)
    ...
    while np.absolute(b_lonlat[1] - c_lonlat[1]) >= err_tolerance or np.absolute(
        ...
        for i in range(0, 10):
            ...
            w1_lonlat = _convert_node_xyz_to_lonlat_rad(w1_new)
            w2_lonlat = _convert_node_xyz_to_lonlat_rad(w2_new)
        ...
        b_lonlat = _convert_node_xyz_to_lonlat_rad(copy.deepcopy(max_section[0]))
        c_lonlat = _convert_node_xyz_to_lonlat_rad(copy.deepcopy(max_section[1]))
...
def min_latitude_rad(v1, v2):
    ...
    while np.absolute(b_lonlat[1] - c_lonlat[1]) >= err_tolerance or np.absolute(
        ...
        b_lonlat = _convert_node_xyz_to_lonlat_rad(copy.deepcopy(min_section[0]))
        c_lonlat = _convert_node_xyz_to_lonlat_rad(copy.deepcopy(min_section[1]))
...
def get_intersection_point_gcr_gcr(w0, w1, v0, v1):
    ...
    """
    w0 = _normalize_in_place(w0)
    w1 = _normalize_in_place(w1)
    v0 = _normalize_in_place(v0)
    v1 = _normalize_in_place(v1)
    norm_w0w1 = _normalize_in_place(np.cross(w0, w1))
    # the cross product of [0,0,1] and [0,0,-1] by np.cross will be [0,0,0]
    if v0 == [0,0,1] and v1 == [0,0,-1]:
        norm_v0v1 = [1,0,0]
    else:
        norm_v0v1 = _normalize_in_place(np.cross(v0, v1))

    if _normalize_in_place(np.cross(norm_w0w1, norm_v0v1).tolist()) == [0,0,0]:
        return [0, 0, 0]
