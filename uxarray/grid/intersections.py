...
def gca_constLat_intersection(gca_cart, constLat, fma_disabled=False):
    ...
    s_tilde = np.sqrt(nx ** 2 + ny ** 2 - np.linalg.norm(n) ** 2 * np.sin(constLat) ** 2)
    p1_x = -(1.0 / (nx ** 2 + ny ** 2)) * (constLat * nx * nz + s_tilde * ny)
    p2_x = -(1.0 / (nx ** 2 + ny ** 2)) * (constLat * nx * nz - s_tilde * ny)
    p1_y = -(1.0 / (nx ** 2 + ny ** 2)) * (constLat * ny * nz - s_tilde * nx)
    p2_y = -(1.0 / (nx ** 2 + ny ** 2)) * (constLat * ny * nz + s_tilde * nx)

    p1 = np.array([p1_x, p1_y, constLat])
    p2 = np.array([p2_x, p2_y, constLat])

    # Now test which intersection point is within the GCA range
    res = np.array([])
    if point_within_gca(p1, gca_cart):
        res = np.append(res, p1)
    elif point_within_gca(p2, gca_cart):
        res = np.append(res, p2)
